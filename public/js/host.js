...
// TokBox Settings
  ...
  $inputMessage.click(function () {
    ...
    session.connect(token, function(error) {
      var publisher = OT.initPublisher('serverVideoBox', publishSettings);
...
/*
  session.on("streamCreated", function(event) {
    ...
    console.log(String(event.stream.name).charAt(0) + ' joined');

    if (event.stream.name == "Parasite"){
    }
    else{
      session.subscribe(event.stream, 'user' + String(event.stream.name).charAt(0), defaultSettings);
    }
   });
*/
});
